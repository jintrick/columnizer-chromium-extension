...
Element.prototype.setTinyText = (text, maxLength = 20) => {
    if (text.length > 20) {
        this.title = text;
...
class NakedHTML {
    ...
    removeWrapper(parent = this.root) {
        ...
        for (const node of children) {
            ...
            //要素別のハンドラを呼び出し
            if (this[`handle_${node.nodeName}`]) this[`handle_${node.nodeName}`](node);
            ...
            const hasOnlyElementChild = child && child.nodeType === Node.ELEMENT_NODE;
            const hasSpecialAttributes = Array.from(node.attributes).some(attr =>
                attr.name.startsWith('data-') ||
                attr.name.startsWith('aria-') ||
                attr.name === 'role');
            if (hasOneChild && hasOnlyElementChild && !hasSpecialAttributes) {
    ...
    handle_IFRAME(iframe) {
        ...
        if (!(title = iframe.contentDocumnet?.title)) return;
